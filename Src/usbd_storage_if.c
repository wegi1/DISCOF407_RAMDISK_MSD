...
#endif
#define STORAGE_CAPPACITY                1024*182
...
uint8_t RamDisk[1024*118];

uint8_t *cram = (uint8_t *)CCMDATARAM_BASE;
...
int8_t STORAGE_Read_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    ...
    if(blk_addr > 235)
    {
        memcpy(buf, (void *)(cram + (STORAGE_BLK_SIZ * (blk_addr-236))), (STORAGE_BLK_SIZ * blk_len));
    }
    else
    {
...
int8_t STORAGE_Write_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
	...
	if(blk_addr > 235)
    {
        memcpy((void *)(cram + (STORAGE_BLK_SIZ * (blk_addr-236))), buf, (STORAGE_BLK_SIZ * blk_len));
    }
    else
    {
...
void STORAGE_FileSystem_Init (void)
{
   uint8_t *ptr8;
	...
	memset(cram, 0, CCMDATARAM_END-CCMDATARAM_BASE+1);
...
//---
    ...
    memcpy((void *)(RamDisk + (STORAGE_BLK_SIZ * 3) ), &BIOS_Parameter_Block.BS_VolLab , (sizeof(BIOS_Parameter_Block.BS_VolLab)));
    RamDisk[(STORAGE_BLK_SIZ * 3) + 11] = 0x08; //disk name attribute
    memcpy((void *)(RamDisk + 32 + (STORAGE_BLK_SIZ * 3) ), &File_Name , (sizeof(File_Name)));
//---
    /*----------- COPY ONE OF TEXTFILE  -----------------*/
    memcpy((void *)(RamDisk + (STORAGE_BLK_SIZ * 4) ), &Text_File_Data , (sizeof(Text_File_Data)));
//---
}
